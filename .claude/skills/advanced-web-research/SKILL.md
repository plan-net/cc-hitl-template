...
> ⚠️ **CRITICAL: NEVER TRUNCATE THE RESEARCH REPORT**
>
> The `full_research_report` field MUST contain the complete, unmodified research report.
> - **NEVER** use slicing like `full_report[:2000]` or any length limits
> - **NEVER** summarize, shorten, or modify the content
> - **NEVER** add `"..."` or truncation markers
> - The full report is essential for traceability, audit, and evaluation purposes
> - Langfuse can handle large payloads (100KB+) without issues
...
# ⚠️ CRITICAL: Send complete report - NO truncation allowed
...
# ⚠️ CRITICAL: Extract FULL report - NO truncation, NO slicing, NO summarization
...
# Validate: Log report length to ensure full content is being sent
print(f"Sending full report to Langfuse: {len(full_report)} characters")

# Create output payload with COMPLETE report (no truncation!)
output_payload = {
    "status": "completed",
    ...
    "full_research_report": full_report  # ⚠️ FULL REPORT - never truncate!
}
...
### Purpose
Generate PPTX presentation via direct Gamma API calls and upload to Digital Ocean Spaces for public access.
...
GAMMA_CONFIG = {
    "textMode": "generate",
    "theme": "Serviceplan_Light",
    "numCards": 12,
    "textOptions": {
        "amount": "detailed",
        "tone": "Professional, analytical, research-focused (American English). Clear and authoritative, emphasizing key insights and data-driven findings. Suitable for researchers, analysts, and strategic decision-makers.",
        "audience": "Researchers, analysts, decision-makers, strategic planners. Professional tone suitable for executive briefings and research reports.",
        "language": "en"
    },
    "imageOptions": {
        "source": "aiGenerated",
        "model": "recraft-v3-svg",
        "style": "Modern illustration style. Alice Zhang x HelloVon Studio crossover: minimalist characters in dynamic poses, clean lines. Multiple perspectives, diverse viewpoints. Abstract data patterns, thought bubbles. Inclusive, human-centered. Avoid stock aesthetic, typography"
    },
    "cardOptions": {
        "dimensions": "16x9"
    },
    "disclaimerCard": "**Welcome to Your Advanced Web Research Report** This presentation contains comprehensive research findings generated by AI-powered web analysis using Exa.ai Research API. **Note:** Both the research and this presentation were generated by AI. Please verify critical information independently before making decisions based on this content.",
    "additionalInstructions": "Welcome/Disclaimer: Use provided card as-is. No imagery. Cover: Concise title (research question). 1-2 sentence intro. One thematic AI image (conceptual, abstract, no text/numbers/logos), right side layout. Executive Summary: 3-5 key findings as bullets. One guiding AI image (thematic, not data-based). Research Insights: One finding per card with claim headline. Chart/table OR image per card—never both. Split long content vs shrinking fonts. Charts: Columns=comparisons, Bars=categories, Lines=trends, Donuts/pies=share (≤6 slices), Tables=complex data. Readable labels, comfortable fit. Images: Use when visualization not possible. Centered, conceptually aligned. No text/logos/numbers. Style: flat/line, minimal, modern. Layout: Headlines 24-32pt, Body 14-16pt. Split if crowded. Keep within margins for PPT export. Preserve: Don't edit quotes, references, links. Keep Report Creation Notice and Research Methodology & Data Sources exactly as provided. Place anything after methodology on single closing card. Export: Ensure nothing cuts off in PPT."
}
```
...
**Use Python to prepare the research report for Gamma:**
...
with open('/tmp/research_report.md', 'r') as f:
    report = f.read()
...
with open('/tmp/research_ids.env', 'r') as f:
    for line in f:
        if line.startswith('RESEARCH_QUESTION='):
            research_question = line.strip().split('=', 1)[1]
            break

# Disclaimer card content
disclaimer = """**Welcome to Your Advanced Web Research Report**

This presentation contains comprehensive research findings generated by AI-powered web analysis using Exa.ai Research API.

**Note:** Both the research and this presentation were generated by AI. Please verify critical information independently before making decisions based on this content."""

# Build the presentation content
lines = []

# Add disclaimer as first card
...
# Add the full research report (Gamma will parse H2 sections)
lines.append(report)

# Add report creation notice at the end
lines.append("\n---\n")
lines.append("## Report Creation Notice\n")
lines.append("AI-generated comprehensive research report created by Advanced Web Research Agent using Exa.ai Research API and Gamma AI presentation generation.\n")

content = "\n".join(lines)

# Save for the API call
with open('/tmp/gamma_input.txt', 'w') as f:
    f.write(content)

print(f"Prepared {len(content)} characters for Gamma presentation")
PYTHON_EOF
```

---

### Step 2: Create Presentation via Gamma API (POST)

**Display progress:**
```markdown
📊 **Generating presentation** (12 slides)...
```

**Execute Gamma API Call:**

```bash
# Read the prepared content
GAMMA_CONTENT=$(cat /tmp/gamma_input.txt)

# Create presentation via Gamma API using Python for proper JSON handling
python3 << 'PYTHON_SCRIPT'
import json
import subprocess
import re
...
# Read the content
with open('/tmp/gamma_input.txt', 'r') as f:
    content = f.read()

# Read research question for title
research_question = "Advanced Web Research Report"
try:
    with open('/tmp/research_ids.env', 'r') as f:
        for line in f:
            if 'RESEARCH_QUESTION' in line:
                research_question = line.strip().split('=', 1)[1] if '=' in line else research_question
                break
except:
    pass
...
# Parse and save generation ID
try:
    data = json.loads(body)
    gen_id = data.get('generationId', '')
    if gen_id:
        with open('/tmp/gamma_generation_id.txt', 'w') as f:
            f.write(gen_id)
        print(f"✅ Presentation generation started: {gen_id}")
    else:
        print(f"❌ No generation ID in response")
...
### Step 3: Poll for Completion (GET)

**Poll every 10 seconds until complete:**

```bash
# Read generation ID
GENERATION_ID=$(cat /tmp/gamma_generation_id.txt 2>/dev/null)
...
echo "⏳ Polling for presentation completion..."
...
for i in {1..30}; do
  ...
  if [ "$STATUS" = "completed" ]; then
    echo "$RESPONSE" > /tmp/gamma_result.json

    # Extract key information
    GAMMA_URL=$(echo "$RESPONSE" | python3 -c "import sys, json; d=json.load(sys.stdin); print(d.get('gammaUrl', ''))" 2>/dev/null)
    CREDITS_DEDUCTED=$(echo "$RESPONSE" | python3 -c "import sys, json; d=json.load(sys.stdin); print(d.get('credits', {}).get('deducted', 0))" 2>/dev/null)
    CREDITS_REMAINING=$(echo "$RESPONSE" | python3 -c "import sys, json; d=json.load(sys.stdin); print(d.get('credits', {}).get('remaining', 0))" 2>/dev/null)

    echo ""
    echo "✅ Presentation generated successfully!"
    echo "Gamma URL: $GAMMA_URL"
    echo "Credits used: $CREDITS_DEDUCTED"
    echo "Credits remaining: $CREDITS_REMAINING"

    # Save URLs for later steps
    echo "GAMMA_URL=$GAMMA_URL" > /tmp/gamma_urls.env
    echo "CREDITS_DEDUCTED=$CREDITS_DEDUCTED" >> /tmp/gamma_urls.env
    echo "CREDITS_REMAINING=$CREDITS_REMAINING" >> /tmp/gamma_urls.env

    break
  elif [ "$STATUS" = "failed" ]; then
    echo "❌ Presentation generation failed"
    echo "$RESPONSE" | python3 -c "import sys, json; d=json.load(sys.stdin); print('Error:', d.get('message', 'Unknown error'))" 2>/dev/null
    break
  fi
...
if [ "$STATUS" != "completed" ] && [ "$STATUS" != "failed" ]; then
  echo "⚠️ Presentation generation timed out after 5 minutes"
fi
```
...
### Step 4: Download PPTX Export

**The Gamma API response includes an `exportUrl` field with the direct PPTX download link:**

```bash
# Read the export URL from the API response (saved in gamma_result.json)
EXPORT_URL=$(cat /tmp/gamma_result.json | python3 -c "import sys, json; print(json.load(sys.stdin).get('exportUrl', ''))")

if [ -z "$EXPORT_URL" ]; then
  echo "⚠️ No export URL in API response"
  exit 0
fi
...
echo "Downloading PPTX from export URL..."

# Download the PPTX (no auth needed for exportUrl)
curl -s -L -o "/tmp/$FILENAME" "$EXPORT_URL"

# Check if file was downloaded correctly
if [ -f "/tmp/$FILENAME" ] && [ -s "/tmp/$FILENAME" ]; then
  FILE_SIZE=$(ls -lh "/tmp/$FILENAME" | awk '{print $5}')
  FILE_TYPE=$(file "/tmp/$FILENAME" | grep -i "zip\|office" || echo "unknown")
  echo "✅ PPTX downloaded: /tmp/$FILENAME ($FILE_SIZE)"
  echo "PPTX_FILE=/tmp/$FILENAME" >> /tmp/gamma_urls.env
  echo "PPTX_FILENAME=$FILENAME" >> /tmp/gamma_urls.env
  echo "EXPORT_URL=$EXPORT_URL" >> /tmp/gamma_urls.env
else
  echo "⚠️ PPTX download may have failed - file is empty or missing"
  echo "Using Gamma URL for direct access instead"
fi
```

**Note:** The `exportUrl` is a direct download link that doesn't require authentication. It's included in the API response when `exportAs: "pptx"` is specified in the request.

---

### Step 5: Upload to Digital Ocean Spaces (CDN)

**Upload the PPTX to Digital Ocean Spaces for public access:**
...
if [ -z "$PPTX_FILE" ] || [ ! -f "$PPTX_FILE" ]; then
  echo "⚠️ No PPTX file to upload - using Gamma URL only"
  exit 0
fi

echo "📤 Uploading to Digital Ocean Spaces..."

# Use Python with boto3 for S3-compatible upload
...
pptx_file = None
pptx_filename = None
with open('/tmp/gamma_urls.env', 'r') as f:
    for line in f:
        if line.startswith('PPTX_FILE='):
            pptx_file = line.strip().split('=', 1)[1]
        elif line.startswith('PPTX_FILENAME='):
            pptx_filename = line.strip().split('=', 1)[1]
...
# Upload path
upload_key = f"research-presentations/{pptx_filename}"
...
try:
    ...
    s3.upload_file(
        ...
        ExtraArgs={
            ...
            'ContentDisposition': f'attachment; filename="{pptx_filename}"'
        }
    )

    # Construct public URL
    public_url = f"https://{bucket}.{region}.digitaloceanspaces.com/{upload_key}"

    print(f"✅ Uploaded to Digital Ocean Spaces!")
    print(f"Public URL: {public_url}")

    # Save the CDN URL
    with open('/tmp/gamma_urls.env', 'a') as f:
        f.write(f"CDN_URL={public_url}\n")

except Exception as e:
    print(f"❌ Upload failed: {e}")
    print("Using Gamma URL for access instead")
PYTHON_UPLOAD
```

**Alternative: Direct curl upload using S3 signature (if boto3 not available):**

```bash
# If boto3 is not available, use curl with AWS signature
# This is a fallback method

source /tmp/gamma_urls.env

if [ -z "$PPTX_FILE" ] || [ ! -f "$PPTX_FILE" ]; then
  echo "⚠️ No PPTX file to upload"
  exit 0
fi
...
if command -v aws &> /dev/null; then
  ...
  CDN_URL="https://studios-general-bucket.fra1.digitaloceanspaces.com/research-presentations/$PPTX_FILENAME"
  echo "CDN_URL=$CDN_URL" >> /tmp/gamma_urls.env
  echo "✅ Uploaded via AWS CLI: $CDN_URL"
else
  echo "⚠️ Neither boto3 nor awscli available - using Gamma URL only"
fi
```
...
**Show the user all available links:**
...
echo "=========================================="
echo ""
echo "**Gamma Editor URL:** $GAMMA_URL"
echo "(Edit and customize your presentation)"
echo ""

if [ -n "$CDN_URL" ]; then
  echo "**Download PPTX:** $CDN_URL"
  echo "(Direct download link - publicly accessible)"
  echo ""
fi
...
**On timeout (polling exceeded 5 minutes):**
```markdown
✅ **Research Analysis Completed Successfully**
...
2. **Retry later:** May succeed if transient error
3. **Manual creation:** Use the research report above to create presentation
...
rm -f /tmp/gamma_input.txt
rm -f /tmp/gamma_payload.json
rm -f /tmp/gamma_generation_id.txt
rm -f /tmp/gamma_result.json
rm -f /tmp/gamma_urls.env
rm -f /tmp/*.pptx
...
workflow_state["gamma_url"] = gamma_url
workflow_state["cdn_url"] = cdn_url  # May be None if upload failed
workflow_state["costs"]["presentation_credits"] = credits_deducted
```
...
**Estimated Size:** 1,850 lines

**Changelog:**
- v2.6 (2025-12-23): **Enhancement** - Added explicit warnings and validation to prevent report truncation in Langfuse output. Added `print()` statement to log report length for verification. Clarified that `full_research_report` must NEVER be truncated.
