...
def load_local_skills(cwd: str) -> List[Dict[str, str]]:
    """
    Load skills from local .claude/skills/ directory for native mode.

    This allows local development without containers by discovering skills
    ...
    # Load skills from local .claude/skills/ directory
    if skills_dir.exists():
        for skill_dir in skills_dir.iterdir():
            if skill_dir.is_dir() and (skill_dir / "SKILL.md").exists():
                ...
                logger.info(f"Loaded local skill: {skill_dir.name}")

    if plugin_specs:
        logger.info(f"Successfully loaded {len(plugin_specs)} local skills")
    else:
        logger.warning("No local skills found")
...
class ClaudeSessionActor:
    ...
    def __init__(self, cwd: Optional[str] = None, permission_mode: str = "acceptEdits"):
        ...
        if is_containerized:
            ...
            # Native execution: Load skills from local .claude/skills/ directory
            logger.info("=" * 70)
            logger.info("LOCAL SKILL LOADING (Native Mode)")
            logger.info("=" * 70)

            working_dir = cwd or os.getcwd()
            plugin_specs = load_local_skills(working_dir)
            ...
            if plugin_specs:
                ...
                logger.warning("No local skills found - agent will run without skills")
                logger.warning(f"Add skills to: {working_dir}/.claude/skills/")

            logger.info("=" * 70)

            # Use project settings for permissions
